...
void battery_init(BATTERY *battery, ADC_HandleTypeDef *hadc)
{
	...
	HAL_ADCEx_Calibration_Start(battery->adc_bat, ADC_SINGLE_ENDED); 
}

static uint16_t get_adc_channel(BATTERY *battery, uint32_t channel)
{
	uint16_t adc = 0;
  ...
  if (HAL_ADC_ConfigChannel(battery->adc_bat, &sConfig) != HAL_OK)
  {
	...
	// Read ADC
		HAL_ADC_Start(battery->adc_bat);
		HAL_ADC_PollForConversion(battery->adc_bat, 1000);
...
static float battery_get_adc_voltage(BATTERY *battery, uint32_t channel)
{
