...
ColumnLayout {
    ...
    Component.onCompleted: {
        ...
        for (let i = 0; i < telemetrySchema.length; i++)
            m[telemetrySchema[i].key] = telemetrySchema[i].visible
        vis = m
    }
    ...
    Rectangle {
        ...
        RowLayout {
            ...
            Button { text: "X+"; flat: true; implicitWidth: 36; onClicked: controller.zoomX(0.7) }
            Button { text: "X−"; flat: true; implicitWidth: 36; onClicked: controller.zoomX(1.43) }
            Rectangle { width: 1; height: 20; color: "#ddd" }
            Button { text: "Y+"; flat: true; implicitWidth: 36; onClicked: controller.zoomY(0.7) }
            Button { text: "Y−"; flat: true; implicitWidth: 36; onClicked: controller.zoomY(1.43) }
            Rectangle { width: 1; height: 20; color: "#ddd" }
            Button { text: "Reset"; flat: true; onClicked: controller.resetView() }
            CheckBox {
                text: "Live"
            ...
            Label {
                text: "Scroll: zoom X · Ctrl+Scroll: zoom Y · Drag: pan"
            ...
            onPaint: {
                ...
                for (let i = 0; i < telemetrySchema.length; i++) {
                    ...
                    if (sig.style === "dash" || n < 3) {
                        // setpoint / ít điểm: nối thẳng
                        for (let k = 2; k < pts.length; k += 2)
                            ctx.lineTo(px(pts[k]), py(pts[k + 1]))
                    } else {
                        // Catmull-Rom spline: đi qua ĐÚNG mọi điểm gốc, chỉ
                        // làm mượt đoạn nối → giữ nguyên bản chất dữ liệu thô.
                        let tension = 0.22
                        for (let j = 0; j < n - 1; j++) {
    ...
    Rectangle {
        ...
        Flow {
            ...
            Repeater {
                ...
                delegate: Rectangle {
                    width: legendRow.implicitWidth + 10
                    height: 24
                    radius: 4
