...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
GPIO_TypeDef   *rgb_ports[] = {GPIOB,GPIOB,GPIOB};
uint16_t         rgb_pins[] = {LED_R_Pin,LED_G_Pin,LED_B_Pin};
RGB rgb;
uint16_t count = 0;
...
{	
	if (&button1 == buttonX || &button2 == buttonX)
	{
		switch(led_status) 
		{
			case LED_OFF:
...
{
	rgb_blink(&rgb, RED,  500);
}
void led_Green_Blink5Hz()
{
	rgb_blink(&rgb, GREEN,  100);
}

void led_Blue_Blink10Hz()
{
	rgb_blink(&rgb, BLUE,  50);
}
...
int main(void)
{
  ...
  /* USER CODE BEGIN Init */
	...
	button_init(&button1, GPIOA, BT1_Pin);
	button_init(&button2, GPIOA, BT2_Pin);
